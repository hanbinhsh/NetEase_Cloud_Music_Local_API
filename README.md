...
GUI 默认扫描 `cloudmusic.dll`，这也是优先推荐的范围；如果该范围找不到，再把扫描范围切到 `all`。候选地址可能显示为 `cloudmusic.dll+0x...`、其他模块名加偏移，或 `abs:0x...`。优先选择模块名加偏移的候选；`abs:0x...` 通常是绝对地址，可能只能在当前客户端进程生命周期内稳定。

网易云界面显示的是整秒，但内存里的 `Double` 可能带小数，例如界面显示 `03:25`，内存值可能是 `205.826667`。定位器默认容差为 `1` 秒；如果仍然扫不到，可以适当调大容差后再用继续筛选收窄。
...
- 如果新版本把播放进度移动到其他模块，GUI 定位器可以切到全可读内存扫描，并保存 `module.dll+offset` 格式的地址。
